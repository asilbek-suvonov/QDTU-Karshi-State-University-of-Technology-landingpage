...
export function NavbarDropdown({ title, links }: NavbarDropdownProps) {
  return (
    <div className="relative group py-6">
      ...
      <div className="absolute top-[calc(100%-12px)] left-1/2 -translate-x-1/2 w-44 bg-card border border-border shadow-2xl p-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 rounded-b-lg -translate-y-2 group-hover:translate-y-0 z-50
      ...
      >
        <div className="pt-1"> {/* Uchburchak va kontent orasidagi masofa */}
          {links.map((link) => (
            <Link
              key={link.title}
              href={link.href}
              className="block px-3 py-2 text-xs font-medium tracking-wide hover:bg-primary/5 hover:text-primary rounded-md transition-all duration-150"
            >
              {link.title}
            </Link>
          ))}
